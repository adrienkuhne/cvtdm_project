...
train.range = predict(range_values, train.set) #Defined previously.
valid.range = predict(range_values, val.set) #Defined previously. #Test.
X_train <- scale(train.range[, c(1:9)])
y_train <- train.range$Potability

dim(y_train) <- c(length(y_train), 1) # add extra dimension to vector

X_test <- scale(valid.range[, c(1:9)])

y_test <- valid.range$Potability
...
layer_size <- getLayerSize(X_train, y_train, hidden_neurons = 4) #The number of hidden neurons should be between the size of the input layer and the size of the output layer. The number of hidden neurons should be 2/3 the size of the input layer, plus the size of the output layer. The number of hidden neurons should be less than twice the size of the input layer. Here, it is therefore 4.
layer_size
```

Neural nets (second try) - Romain.
...
scale_back_min_max_score <- function(x_) {
  min_x = min(train.range$Potability)
  max_x = max(train.range$Potability)
  x = sapply(x_, function(x__) x__*(max_x - min_x) + min_x)
  return(x)
}
nn1 <- neuralnet(Potability ~ . ,data = train.range, linear.output = FALSE, hidden = 2) 
plot(nn1, rep = "best")
nn1.predict.train <- predict(nn1, train.range)
nn1.predict.valid <- predict(nn1, valid.range)
rmse.nn1.train <- rmse(actual = scale_back_min_max_score(train.range$Potability),
                       predicted = scale_back_min_max_score(nn1.predict.train))
rmse.nn1.valid <- rmse(actual = scale_back_min_max_score(valid.norm.df$Price),
                       predicted = scale_back_min_max_score(nn1.predict.valid))

#The number of hidden neurons should be between the size of the input layer and the size of the output layer. The number of hidden neurons should be 2/3 the size of the input layer, plus the size of the output layer. The number of hidden neurons should be less than twice the size of the input layer. Here, the size of the input layer is 9 and the size of the output layer is 2. Therefore, the number of hidden neurons should be 5. #Source : https://stats.stackexchange.com 
```

Logistic Regression - Romain

```{r}
logit.reg = glm(Potability ~ ., data = train.range, family = "binomial")
options(scipen=999)
summary(logit.reg)

pred.logit.reg = predict(logit.reg, valid.range[,-10], type = "response")
head(sort(pred.logit.reg,decreasing = T),5)

fourfoldplot(confusionMatrix(as.factor(ifelse(pred.logit.reg > 0.5, 1, 0)), valid.range$Potability)$table) #Option I think.

results.df = data.frame(actual = valid.range$Potability, logit.reg = as.factor(ifelse(pred.logit.reg > 0.5, 1, 0)), logit.reg.prob = pred.logit.reg)

accuracy.df = data.frame(logit.reg=confusionMatrix(as.factor(ifelse(pred.logit.reg > 0.5, 1, 0)), valid.range$Potability)$overall[1])
...
Other parameters:
```{r}
rf2=randomForest(Potability ~ ., data = train.range, ntree = 500, mtry = 4, nodesize = 5, importance = TRUE)
varImpPlot(rf2, type = 1) #Optional I think.
pred.rf2=predict(rf2, valid.range, type = "class")
rf.cm2=confusionMatrix(pred.rf2, valid.range$Potability) 
rf.cm2
