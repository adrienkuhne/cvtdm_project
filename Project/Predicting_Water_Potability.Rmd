...
The best k that maximizes our accuracy is k = 18

```{r}
library(class)
knn_res = knn(train.range[-10], valid.range[-10], cl=train.range$Potability,k=18, prob = T)
...
```{r}
nn_predict = predict(neural.net_cv$finalModel, valid.range, type = "class")
nn_predict_proba = predict(neural.net_cv$finalModel, valid.range, type = "raw")
confusion_nn_cv =confusionMatrix(as.factor(nn_predict), valid.range$Potability, positive = "1")
confusion_nn_cv
accuracy_nn = confusionMatrix(as.factor(nn_predict), valid.range$Potability, positive = "1")$overall["Accuracy"]
