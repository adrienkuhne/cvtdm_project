...
We can see that we have more non-potable than potable water. In general, even if we have more non-potable water, our data set is pretty balanced. We have 1278 potable and 1998 non-potable water observations. We think we can focus our analysis on accuracy and F1 score to be more robust.
...
Without dropping values and by using a decision tree, the accuracy of our model using a 5 folds cross-validation is 0.6327534.
...
### Creating a function to scale back our train and test set from the normalisation of our data.
...
We can see that they are all stacked together. It does nott seem like there is a separation between both groups. Therefore, it will be hard to classify them correctly using a model. But looking at this graph, we can clearly see that our data are not optimal.
...
The best K that maximizes our accuracy is K = 18. The results of the average accuracy from our cross-validation using K = 18 is 0.6317459. We could replace this value (K=18) into our test set.
...
As we can see, the accuracy of the average 5 folds cross-validation performed on our training set is very close to our test set. Therefore, we do not think that we are overfitting since the accuracy is quite similar for both situations.
...
Pruning our full grown tree using 5 folds cross-validation : 
```{r}
library(rattle)
...
With a decision tree, we obtain an accuracy of 0.6344, which is better than with K-NN ! The balanced accuracy is 0.5690. It means that we have a little bit better than guessing at random. It is seems reasonable to obtain those kind of results since our data set was probably generated by a software. 
...
### Naive bayes.
With Naive bayes, we do not need to scale.
```{r}
library(e1071)
...
We obtain an accuracy of 0.6237 and a balanced accuracy 0.555. We still struggle to predict well.
...
We get the same results on our test set whether we use Naives bayes cross-validation or simply Naives bayes.
...
The largest value of accuracy (0.6337712) was used to select the optimal model.
The final values and method used for the model were respectively nIter (150 trees) and Adaboost.M1.
...
We obtain an accuracy of 0.6427 and a balanced accuracy of 0.6063. It is better than the other methods so far. 
...
Unfortunately, we cannot use any tuning parameters for this method (but Bagged AdaBoost has !).
The accuracy of the 5 folds cross-validation is equal to 0.6541236 (using 200 trees). 

We could verify on our test data.
```{r}
set.seed(1)
...
We obtain an accuracy of 0.6687 and a balanced accuracy of 0.6187. It is improving.
...
When we perform a cross-validation on random forest using the scaled back data, we obtain that the best mtry to use is 6. We find an accuracy equal to 0.6597099.
...
With the random forest method, we end up with an accuracy of 0.674 and a balanced accuracy of 0.6228. It corresponds to the best coefficients of accuracy that we had so far!
...
As we can see with the cross-validation, we should use 5 nodes, 1 hidden layer and the decay equal to 0.1. The decay is the regularization parameter that avoids overfitting.
...
We find an accuracy equal to 0.6855. It is the best coefficient that we have seen so far ! 
