...
confusionMatrix(classificationtree_valid_predict, valid_scaled_back$Potability, positive = "1")
```
...
library(adabag)
set.seed(1)
boost = boosting(Potability~., data = train_scaled_back, type = "class")

valid_predict_boost = predict(boost, valid_scaled_back , type = "class")
confusionMatrix(as.factor(valid_predict_boost$class), valid_scaled_back$Potability, positive = "1")
```

Accuracy : 0.6359,Balanced Accuracy : 0.5943    Alreadyd way better.    
...
#valid_predict_boost
confusionMatrix(as.factor(valid_predict_bag$class), valid_scaled_back$Potability, positive = "1")
```
...
set.seed(1)
randomf = randomForest(Potability~., data = train_scaled_back,mtry = 7 )


valid_predict_rf = predict(randomf, valid_scaled_back , type = "class")
confusionMatrix(as.factor(valid_predict_rf), valid_scaled_back$Potability, positive = "1")
```

mtry = 7 Accuracy : 0.6725   ,Balanced Accuracy : 0.6176 ### best one I tried will all mtry possibility

with range data
```{r}
set.seed(1)
randomf = randomForest(Potability~., data = train.range,mtry = 5 )


valid_predict_rf = predict(randomf, valid.range , type = "class")
confusionMatrix(as.factor(valid_predict_rf), valid.range$Potability, positive = "1")
```
