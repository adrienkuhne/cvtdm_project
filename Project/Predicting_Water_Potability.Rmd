...
Now let's look at the difference between our Potable and non-potable water using their mean, maximum and minimum value.
...
Here we used the maximum and 0 for Potability  = 1 and potability = 0 as the range of the of the radar chart. We used the mean of each feature for potability = 1 and 0 to represent their characteristics. We can see that there's not that much difference between them
...
In reality, we can see that there's not really a difference between bad and good water which is weird ! We would expect at least some difference in some features.

let's look at the correlation of our variables : 
```{r}
pheatmap(cor(df, use = "complete"), display_numbers = T,cluster_rows = F, cluster_cols = F, fontsize_number = 15)
```

They are not correlated at all between each other meaning that we won't have an issue of multicolinearity ! 
...
Without dropping missing values :3276 obs, 10 variables, Potability = 1 : 1278, 0: 1998
dropping values : 2011 obs, 10 variables, potability = 1 811, 0, 1200 
We lose 1265 obs. It seems that we more missing values in potability = 0
Around 23 % of our Sulfate observations have missing values and 15% for ph which is a lot ! We only have 5% of missing values for Trihalomethanes which is already better than the other but still. We will need to deal with those missing values and try to find the best method :
- Drop them
- Keep them
- Impute them by the median
- impute them using K-NN.


First let's look if the missing values have a pattern :
...
Well it's hard to say but it looks like the missing observations are missing at random. They don't really follow a pattern.

#### We will now need to think to either drop those missing values or to impute them. We will use different imputation techniques on a train set so that we avoid data leackage and we will use a 5 cross validation to validate our results.

Partitioning our data: we will use the stratified sampling method because I want to ensure that i have the same proportion of water quality in each of our samples. We will create a train set that represents 60% our of data and a validation set that will represents 40 % of our data.
```{r}
set.seed(1)
...
#### Cross validation with 5 folds for imputation : We are going to use a decision tree since they handle missing values 

Here without dropping our missing values 
```{r}
set.seed(1) 
...
K-NN imputation method during our cross-validation
```{r}
set.seed(1) 
...
results_cv_impute %>% gather(Attributes, Values, 1:2) %>% ggplot(aes(x = Attributes, y = Values, fill = Method)) + scale_fill_viridis_d() + geom_bar(stat = "identity",position = "dodge") + geom_text(mapping= aes(x = Attributes, y=Values,label = round(Values,4)), vjust=-0.5, size = 2,position = position_dodge(width = .9), col = "red") + labs(title = "Comparaison of Missing values imputation : Dropping, keeping, imputing with median or K-NN", x = "Metrics") + theme_light()
```


Looking at those results it seems that knn imputation did the best in accuracy and F1 score compared to the Median imputation but we can see that the difference is small. Also not dropping variables leads to a better F1 score in general but it's very close to the K-nn imputation method and median method. The problem is that we will need to either impute them or drop the missing value to perform for example a K-NN model or a neural nets.So we will chose the one that maximizes F1 and accuracy and it is the K-NN imputation method !  


### Preprocessing : we will make 2 types of preprocessing. One using the range method and one using the center and scale method and another one using the range method (for neural net) but it's always good so we can compare the 2 methods.
...
#### Let's look at the difference in the distribution when we imputed the missing values with knn and without doiing it :
```{r}
library(reshape2)
...
The distribution didn't really change that much except for Sulfate where we had 25% of missing values but this was predicted. In general our distribution became a little bit more centered but it doesn't seem like there's a huge impact.

3D graph to represent our water quality based on ph, Organic_carbon and Turbidity.
```{r}
